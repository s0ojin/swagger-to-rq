...
program
	...
	.option("-k, --key <string>", "Google Gemini API Key") // 👈 콘솔 가이드 변경
	...
	.action(async (apiPath: string, options) => {
		...
		// 👈 에러 메시지들을 Gemini 기준으로 친절하게 전면 교체!
		if (!apiKey) {
			console.error("❌ 에러: Gemini API Key가 없습니다. -k 옵션을 주거나 .env에 GEMINI_API_KEY를 설정하세요.");
			return;
		}
		...
		try {
			...
			// 👈 여기 콘솔 문구도 구글 감성으로 교체!
			console.log("🤖 3. Google Gemini 무료 엔진 가동 및 TypeScript 코드 생성 중...");
...
`;
			...
			if (!generatedCode) {
				console.error("❌ 에러: Gemini AI가 코드를 생성하는 데 실패했습니다.");
				return;
			}
