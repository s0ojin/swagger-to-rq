...
program
	.name("gen-rq")
	.description("Swagger 명세를 기반으로 TypeScript Interface와 TanStack Query 훅을 자동 생성합니다. (사내 POST 통일 환경 완벽 대응)")
	.version("1.0.2");

program
	.argument("<apiPath>", "변환할 API 엔드포인트 경로 (예: /api/v1/user/search)")
	.option("-s, --swagger <url>", "회사의 Swagger JSON URL 주소")
	.option("-k, --key <string>", "OpenAI API Key")
	.option("-t, --type <string>", "강제 지정 타입 선택 (query 또는 mutation)")
	.action(async (apiPath: string, options) => {
		const apiKey = options.key || process.env.GEMINI_API_KEY || process.env.OPENAI_API_KEY;
		const swaggerUrl = options.swagger || process.env.SWAGGER_URL;
		const forceType = options.type; // 'query' 또는 'mutation'
		...
		if (!apiKey) {
			console.error("❌ 에러: OpenAI API Key가 없습니다. -k 옵션을 주거나 .env에 OPENAI_API_KEY를 설정하세요.");
			return;
		}
		...
		try {
			...
			// Swagger 사내 환경 대응: GET이든 POST이든 경로 내에 존재하는 스펙을 똑똑하게 찾아냄
			...
			// 해당 경로에 등록된 모든 HTTP Method 스펙 파싱 (보통은 post 하나만 있거나 하겠지만 전체 추출)
			...
			console.log("🤖 3. AI 엔진 가동 및 TypeScript 코드 생성 중...");
			const openai = new OpenAI({ apiKey, baseURL: "https://generativelanguage.googleapis.com/v1beta/openai/" });

			// 👈 무기 1: AI에게 사내 특수 환경(모든 API가 POST임)을 학습시키는 프롬프트 튜닝
			const systemPrompt = `
...
`;
			...
			// 사용자가 터미널에서 수동으로 타입을 지정해 주었을 때의 힌트 추가 메시지
			const userPrompt = `
다음 Swagger API 명세를 바탕으로 코드를 생성해줘:
${targetedSpec}

${forceType ? `[🚨 개발자 수동 강제 사항]: 이 API는 문맥 분석 결과를 무시하고, 무조건 TanStack Query의 'use${forceType.charAt(0).toUpperCase() + forceType.slice(1)}' 형태로 코드를 생성해야 합니다.` : ""}
`;
			...
			const response = await openai.chat.completions.create({
				...
				temperature: 0.1, // 코드의 일관성을 극대화하기 위해 온도를 더 낮춤
			});
			...
			console.log(`\n✨ 성공! 사내 컨벤션이 적용된 TypeScript Interface와 Query 훅 파일이 탄생했습니다.`);
			...
			console.error("❌ 실행 중 치명적인 오류가 발생했습니다:", error.message);
		}
	});
